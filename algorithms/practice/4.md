...
Let $P(n)$ be the above claim, i.e. that given $n$ numbers $A$ and a range $\left[ l, u \right]$, my algorithm returns the number of interval sums on $A$ that are between $\left[ l, u \right]$ via the following calculation:

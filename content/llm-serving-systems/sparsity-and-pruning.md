...
This note surveys where sparsity shows up in LLMs (weights, activations, KV cache) and the pruning techniques that exploit each kind. Like [[llm-serving-systems/quantization|Quantization]], sparsity changes the model representation to cut serving cost, and whether the cut turns into actual speedup depends on the hardware bottlenecks described in [[llm-serving-systems/performance-modeling|Performance Modeling]].
...
Attention weights at inference are extremely sparse in practice; the lecture cites attention matrices over 95% sparse with under 1% of weights carrying meaningful mass. That makes the KV cache (the memory-bound side of decoding, see [[llm-serving-systems/memory-management|Memory Management]]) a natural target.
...
- [[llm-serving-systems/quantization|Quantization]]

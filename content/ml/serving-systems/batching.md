...
An H100 needs a GEMM batch size around 333 before it becomes compute bound (see the critical intensity derivation in [[ml/serving-systems/performance-modeling|Performance Modeling]]), so a serving system that decodes one request at a time wastes almost all of its FLOPs. Batching recovers that throughput. The cost is latency, and every batching strategy is a different answer to how much latency you pay and where.

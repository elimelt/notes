...
This note builds a performance model for LLM serving: the roofline model for single-kernel analysis, then whole-system execution time models for memory, compute, and network. A compact version of the core model lives in the [[ml/serving-systems/roofline-reference|Roofline reference]]. The same model guides [[ml/serving-systems/optimizing-gpu-kernels|kernel optimization]] and system-level choices such as [[ml/serving-systems/batching|batching]].
...
For the batched GEMMs in LLM serving, $M$ is the batch dimension, so matmul on the H100 goes compute bound around batch 333. This single number drives the batching pressure throughout [[ml/serving-systems/batching|Batching]].
...
- [[ml/serving-systems/roofline-reference|Roofline reference]]

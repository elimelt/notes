...
ARP ([RFC 826](https://datatracker.ietf.org/doc/html/rfc826)) maps IP addresses to MAC addresses. IP forwarding decides which machine on the local network should get the packet, and ARP supplies the link layer address the frame needs to actually reach it (see [[networks/3-network/internetworking|internetworking]] for the forwarding side). ARP sits directly on top of the link layer and uses no servers or routers, which it can't anyway, since a host needs ARP working before it can talk to anything by IP.
...
This slide shows where the fields of an outgoing frame come from. The NIC provides the source MAC, ARP provides the destination MAC, and [[networks/3-network/DHCP|DHCP]] provided the host's own IP in the first place.

...
- Service model (datagrams vs virtual circuits, see [[networks/3-network/networking-services|networking services]])
- Addressing
- QoS (priorities)
- Packet size
...
The header format below is from [RFC 791](https://datatracker.ietf.org/doc/html/rfc791). The TTL and protocol fields matter later: TTL expiry is what [[networks/3-network/ICMP|ICMP]] Time Exceeded messages report, and the protocol field says whether the payload is TCP, UDP, or something else.
...
When a host wants to send a packet, it first checks whether the destination IP is on the same network by matching the subnet. If it is, the host sends the packet directly over the link layer, using [[networks/3-network/ARP|ARP]] to resolve the IP address to a MAC address. Otherwise it hands the packet to its default gateway, a router.
...
- [[networks/3-network/networking-services|networking services]]

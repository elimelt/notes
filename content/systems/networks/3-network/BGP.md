...
You can view the internet as a graph of interconnected Autonomous Systems (ASes). Each AS acts on its own and handles routing within itself. The routers inside an AS connect to each other over high-speed links and reach other ASes through border routers. See [[systems/networks/3-network/global-internet|the global Internet]] for why the internet is organized this way.

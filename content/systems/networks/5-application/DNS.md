...
DNS runs over UDP on port 53, see [[systems/networks/4-transport/UDP|UDP]]. Reliability comes from the client retransmitting on timeout, with a 16-bit ID field linking each response to its query. Servers are replicated for load and availability, queries can return multiple records, and the client picks which to use.

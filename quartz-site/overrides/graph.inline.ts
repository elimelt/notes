...
async function renderGraph(graph: HTMLElement, fullSlug: FullSlug) {
  ...
  function renderSvgGraph() {
    ...
    const viewport = select(svg).append("g")
    ...
    const nodeSelection = viewport
      ...
      .attr("stroke-width", (d) => (d.id.startsWith("tags/") ? 2 : 0))
      .attr("vector-effect", "non-scaling-stroke")
      .style("cursor", "pointer")
      .on("mouseover", (_event, d) => {
        updateHoverInfo(d.id)
        renderSvgFromD3()
      })
      .on("mouseout", () => {
    ...
    const labelSelection = viewport
      ...
      .style("fill", computedStyleMap["--dark"])
      .style("font-family", computedStyleMap["--bodyFont"])
      .style("pointer-events", "none")
      .attr("transform", `scale(${1 / scale})`)

    function renderSvgFromD3() {
      const scaledOpacity = Math.max((currentTransform.k * opacityScale - 1) / 3.75, 0)

      linkSelection
        .attr("stroke", (d) =>
          hoveredNodeId !== null && (d.source.id === hoveredNodeId || d.target.id === hoveredNodeId)
      ...
      nodeSelection
        .attr("fill", (d) => (d.id.startsWith("tags/") ? computedStyleMap["--light"] : color(d)))
        .attr("stroke", (d) =>
          d.id.startsWith("tags/") ? computedStyleMap["--tertiary"] : "transparent",
        )
        .attr("opacity", (d) =>
          hoveredNodeId !== null && focusOnHover ? (hoveredNeighbours.has(d.id) ? 1 : 0.2) : 1,
        )
      ...
      labelSelection.attr("opacity", (d) => {
        ...
        if (hoveredNodeId !== null && hoveredNeighbours.has(d.id)) {
          return 1
        }
    ...
    function animate() {
      ...
      nodeSelection.attr("cx", (d) => d.x! + width / 2).attr("cy", (d) => d.y! + height / 2)

      labelSelection
        .attr("x", (d) => d.x! + width / 2)
        .attr("y", (d) => d.y! + height / 2 - nodeRadius(d) * 3)
